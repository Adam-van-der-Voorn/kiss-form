...
describe('touched and dirty', () => {
    it('can register an array item being touched', () => {
        cy.visit('/');
        ...
        cy.get('#form-touched')
            .innerHTMLEqualsObj({ a: false, b: false }, 'fav.pokerHands.0');

        cy.log('clicking in and out of pokerhand A');
        cy.get('[name^="fav.pokerHands.0.a"]')
            .click();
        cy.get('body')
            .click(3, 3);
        ...
        cy.get('#form-touched')
            .innerHTMLEqualsObj({ a: true, b: false }, 'fav.pokerHands.0');
    });
});
